...
sqs = boto3.resource('sqs') # TODO: get configuration

def get_scatter_queue(job_id: JobId):
    return sqs.get_queue_by_name(QueueName="banyan_" + job_id + "_scatter.fifo")

def get_sgather_queue(job_id: JobId):
    return sqs.get_queue_by_name(QueueName="banyan_" + job_id + "_gather.fifo")

def receive_next_message(queue_name):
    global jobs

    queue_url = sqs.get_queue_by_name(queue_name)
    ...
    if content.startswith("EVALUATION_END"):
        # print(content[15:]) #TODO: is this supposed to be there?
        ...
        print(content[12:]) # TODO: is this supposed to be there?
