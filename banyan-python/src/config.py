...
banyanconfig # Global variable representing configuration
...
def configure(
    ...
    # Find user_id
    if user_id is None:
        user_id_env = os.getenv("user_id", default = None)
        if user_id_env is None:
            user_id = loaded_config["user_id"]
        else:
            user_id = user_id_env
    if user_id != loaded_config["user_id"]:
        config_change = True
    banyanconfig["user_id"] = user_id

    # Find api_key
    if api_key is None:
        api_key_env = os.getenv("api_key", default = None)
