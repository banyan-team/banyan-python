...
from future import create_future, Future
...
def get_location(o)->Location:
    if isinstance(o, ValueId):
        return get(get_session().locations, o, NOTHING_LOCATION)
    elif isinstance(o, Future):
        return get(get_session().locations, o.value_id, NOTHING_LOCATION)
...
def create_new_future(source:Location, mutate_from:Future, datatype:str):
    ...
    if source_src_name == "None" or source_src_name ==  "Client" or source_src_name ==  "Value":
...
def create_future_from_sample(value:Any, datatype:String)->Future:
    # TODO: Store values in S3 instead so that we can read from there
    location:Location = Value(value) if total_memory_usage(value) ≤ 4 * 1024 else Client(value)
...
def Future(
    ...
    source:Location = None(), mutate_from:Union[Future,Nothing]=None,
    from_future:Union[AbstractFuture,NoneType] = None, mutation:Any=identity
):
    if from_future is not None:
        create_future_from_existing(convert(Future, from_future):Future, mutation)
    elif isinstance(value, NothingValue):
        create_new_future(source, NOTHING_FUTURE is mutate_from is None else mutate_from, datatype)
    else:
