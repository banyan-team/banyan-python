...
def end_session(
    ...
    **kwargs,
    ...
    send_request_get_response("end_session", request_params)
...
def get_sessions(cluster_name=None, status=None, limit=-1, *args, **kwargs):
    ...
    if cluster_name is not None:
        filters[
            "cluster_name"
        ] = cluster_name  # mapping from key 'cluster_name' -> actual value of the parameter cluster_name
    ...
    if limit > 0:
        ...
        indiv_response = send_request_get_response(
            "describe_sessions", {"filters": filters, "limit": limit}
        )
        ...
        # Get all sessions
        indiv_response = send_request_get_response(
            "describe_sessions", {"filters": filters}
        )
