...
def check_worker_stuck_error(
    ...
    error_for_main_stuck:Union[NoneType, str],
    error_for_main_stuck_time:Union[NoneType, DateTime]
)->Tuple[Union[NoneType,str],Union[NoneType,DateTime]]:
    value_id:ValueId = message["value_id"]
    if value_id == "-2" and error_for_main_stuck_time is None:
        error_for_main_stuck_msg:str = from_jl_value_contents(message["contents"]:str)
...
def check_worker_stuck(
    ...
    if error_for_main_stuck is not None and error_for_main_stuck_time is not None and (Dates.now() - error_for_main_stuck_time) > Second(10):
        print(error_for_main_stuck)
        # TODO: @warn "The above error occurred on some workers but other workers are still running. Please interrupt and end the session unless you expect that a lot of logs are being returned."
...
def _partitioned_computation_concrete(fut:Future, destination:Location, new_source:Location, sessions:Dict[SessionId,Session], session_id:SessionId, session:Session, resource_id:ResourceId, destroyed_value_ids:List[ValueId]):
    # TODO: @partitioned fut begin end

    # Get all tasks to be recorded in this call to `compute`
    tasks:List[DelayedTask] = DelayedTask[req.task for req in session.pending_requests if req isa RecordTaskRequest]
    tasks_reverse:List[DelayedTask] = deepcopy(tasks)
    ...
    for t in tasks:
        ...
        set_task(t)
        if t.partitioned_with_func is not identity:
            ...
            for previous_pa in pa_union_reverse:
                for value_id in pa.partitions.pt_stacks.keys():
                    ...
                    if value_id not in previous_pa.partitions.pt_stacks:
                        ...
                        previous_pa.partitions.pt_stacks[value_id] = deepcopy(pa.partitions.pt_stacks[value_id])
                        ...
                        for constraint in pa.constraints.constraints:
                            ...
                            if len(constraint.args) is not 0:
                                for arg in constraint.args:
                                    arg_v:ValueId = arg[1]
                                    if arg_v == value_id:
                                        copy_constraint = True
                            elif len(constraint.co_args) is not 0:
                            ...
                            if copy_constraint:
                                previous_pa.constraints.constraints.append(deepcopy(constraint))
    ...
    error_for_main_stuck:Union[NoneType, str] = None
    error_for_main_stuck_time:Union[NoneType, DateTime] = None
    partial_gathers: Dict[ValueId,str] = {}
    while True:
